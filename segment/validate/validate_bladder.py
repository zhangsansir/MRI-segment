...
net.load_state_dict(torch.load("../checkpoint/unet_depth=2_fold_1_dice_162520.pth"))
net.eval()
...
def auto_val(net):
    # 效果展示图片数
    dices = 0
    class_dices = np.array([0] * (num_classes - 1), dtype=float)
    ...
    for (input, mask), file_name in tqdm(val_loader):
        ...
        # gt
        # palette = [[0, 0, 0], [246, 16, 16], [16, 136, 246]]#使用后保存为彩色图
